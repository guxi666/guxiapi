...
export default function SettingsPaymentGateway(props) {
  ...
  const [inputs, setInputs] = useState({
    ...
    MinTopUp: 1,
    USDExchangeRate: 7.3,
    USDExchangeRateAutoUpdateEnabled: true,
    USDExchangeRateSyncMinutes: 30,
  });
      ...
      const currentInputs = {
        ...
        USDExchangeRate:
          props.options.USDExchangeRate !== undefined
            ? parseFloat(props.options.USDExchangeRate)
            : 7.3,
        USDExchangeRateAutoUpdateEnabled:
          props.options.USDExchangeRateAutoUpdateEnabled !== undefined
            ? `${props.options.USDExchangeRateAutoUpdateEnabled}` === 'true'
            : true,
        USDExchangeRateSyncMinutes:
          props.options.USDExchangeRateSyncMinutes !== undefined
            ? parseInt(props.options.USDExchangeRateSyncMinutes, 10)
            : 30,
      };
  ...
  const submitPayAddress = async () => {
    ...
    try {
      ...
      if (inputs.USDExchangeRate !== '') {
        options.push({
          key: 'USDExchangeRate',
          value: inputs.USDExchangeRate.toString(),
        });
      }
      options.push({
        key: 'USDExchangeRateAutoUpdateEnabled',
        value: `${!!inputs.USDExchangeRateAutoUpdateEnabled}`,
      });
      if (inputs.USDExchangeRateSyncMinutes !== '') {
        options.push({
          key: 'USDExchangeRateSyncMinutes',
          value: inputs.USDExchangeRateSyncMinutes.toString(),
        });
      }
          ...
          >
            ...
            <Col xs={24} sm={24} md={12} lg={12} xl={12}>
              <Form.InputNumber
                ...
                precision={4}
                label={t('美元汇率（USD/CNY）')}
                placeholder={t('例如：7.2')}
              />
            </Col>
            <Col xs={24} sm={24} md={12} lg={12} xl={12}>
              <Form.InputNumber
                field='USDExchangeRateSyncMinutes'
                precision={0}
                min={1}
                label={t('汇率自动更新时间（分钟）')}
                placeholder={t('例如：30')}
            ...
            <Col xs={24} sm={24} md={24} lg={24} xl={24}>
              <Form.Switch
                field='USDExchangeRateAutoUpdateEnabled'
                label={t('启用美元汇率自动更新')}
              />
            </Col>
          </Row>
