...
const renderUsageMode = (record, t) => {
  if ((record.usage_mode || 1) === 2) {
    return (
...
export const getRedemptionsColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      render: (text, record) => <div>{renderUsageMode(record, t)}</div>,
    },
    {
      title: t('额度'),
      dataIndex: 'quota',
    ...
    },
    {
      ...
      render: (text, record) => {
        const used = parseInt(record.used_count || 0, 10);
        const max = parseInt(record.max_use_count || 0, 10);
        if ((record.usage_mode || 1) === 2) {
          return <div>{max > 0 ? `${used}/${max}` : `${used}/∞`}</div>;
        }
        return <div>{used}</div>;
      },
    },
