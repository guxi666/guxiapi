...
func SearchUsers(c *gin.Context) {
	...
	var quotaMin *int
	var quotaMax *int
	if raw := strings.TrimSpace(c.Query("quota_min")); raw != "" {
		if val, err := strconv.Atoi(raw); err == nil {
			quotaMin = &val
		}
	}
	if raw := strings.TrimSpace(c.Query("quota_max")); raw != "" {
		if val, err := strconv.Atoi(raw); err == nil {
			quotaMax = &val
		}
	}
	pageInfo := common.GetPageQuery(c)
	users, total, err := model.SearchUsers(keyword, group, quotaMin, quotaMax, pageInfo.GetStartIdx(), pageInfo.GetPageSize())
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
// ManageUsersBatch 管理员批量管理用户额度
func ManageUsersBatch(c *gin.Context) {
	...
	if req.Action != "add_quota" && req.Action != "clear_quota" {
		common.ApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	}
	if req.Action == "add_quota" && req.Value <= 0 {
		common.ApiErrorI18n(c, i18n.MsgUserQuotaChangeZero)
		return
	}

	targetIDs := uniqueIntSlice(req.UserIds)
	if req.SelectAll {
		var err error
		targetIDs, err = findBatchTargetUserIDs(req)
		if err != nil {
			common.ApiError(c, err)
			return
		}
	...
	for _, uid := range targetIDs {
		...
		case "add_quota":
			...
			model.RecordLogWithAdminInfo(user.Id, model.LogTypeManage,
				fmt.Sprintf("管理员批量增加用户额度 %s", logger.LogQuota(req.Value)), adminInfo)
		case "clear_quota":
			if user.Quota > 0 {
				if err := model.DecreaseUserQuota(user.Id, user.Quota, true); err != nil {
					failedCount++
					continue
				}
			...
			model.RecordLogWithAdminInfo(user.Id, model.LogTypeManage,
				"管理员批量清空用户额度", adminInfo)
		}
		successCount++
	}

	if successCount == 0 {
		common.ApiErrorMsg(c, "批量操作失败，请检查筛选条件或权限")
		return
	}
...
func findBatchTargetUserIDs(req ManageBatchRequest) ([]int, error) {
	...
	if req.QuotaMin != nil {
		query = query.Where("quota >= ?", *req.QuotaMin)
	}
	if req.QuotaMax != nil {
		query = query.Where("quota <= ?", *req.QuotaMax)
	}
